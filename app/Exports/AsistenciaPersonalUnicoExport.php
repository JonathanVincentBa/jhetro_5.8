...
class AsistenciaPersonalUnicoExport implements FromView
{
    use Exportable;
    ...
    public function view(): View
    {
        ...
        $usuarios=DB::select('SELECT users.name, acceso.ip_client, acceso.date, acceso.condicion
                            ...
                            AND users.name = "'.$this->nombre.'"
                            ORDER BY users.name, acceso.date asc');
        //dd($usuarios);
        return view('seguridad.empleado.reportePersonalUnicoExcel',compact('usuarios'));
    }
}
