...
@section ('contenido')
	<div class="row">
		<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
			<h3>Editar Usuario: {{$usuario->name}} 
				<a href="{{URL::action('UsuarioController@index')}}"><button class="btn btn-danger">Cancelar</button></a>
			</h3>
			@if (count($errors)>0)
	...
	<form method="POST" action="{{ url("seguridad/usuario/{$usuario->id}") }}">
		@csrf
		@method('PUT')
					...
					<select name="id_rol" id="id_rol" class="form-control selectpicker" data-live-search="true" title="Seleccione un rol..." requerid>
						@foreach ($roles as $rol)
								@if( $rol->id_rol==$usuario->id_rol )
			                        	<option value="{{ $rol->id_rol }}" selected>{{ $rol->descripcion }}</option>
			                        @else
